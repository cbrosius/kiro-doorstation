...
void hardware_test_init(void) {
  ...
  // Create timeout task
  BaseType_t result =
      xTaskCreate(hardware_test_door_timeout_task, "hw_test_timeout", 2048,
                  NULL, 5, &test_ctx.timeout_task_handle);
...
esp_err_t hardware_test_door_opener(uint32_t duration_ms) {
  ...
  // Activate door relay
  hw_status_log_event(HW_EVENT_DOOR_OPEN, duration_ms, "Web Test");
