...
// Global OTA context
static ota_context_t g_ota_ctx = {.state = OTA_STATE_IDLE,
                                  .total_size = 0,
                                  .written_size = 0,
                                  .progress_percent = 0,
                                  .error_message = {0},
...
void ota_handler_init(void) {
  ...
  if (running != NULL) {
    ESP_LOGI(TAG, "Running partition: %s at offset 0x%lx", running->label,
             running->address);
  }
...
void ota_get_info(ota_info_t *info) {
  ...
  if (app_desc != NULL) {
    ...
    strncpy(info->idf_version, app_desc->idf_ver,
            sizeof(info->idf_version) - 1);
  } else {
  ...
  ESP_LOGI(TAG,
           "Firmware info - Version: %s, Date: %s, IDF: %s, Partition: %s, Can "
           "rollback: %s",
           info->version, info->build_date, info->idf_version,
           info->partition_label, info->can_rollback ? "yes" : "no");
}

esp_err_t ota_validate_image(const uint8_t *data, size_t length,
  ...
  if (length < sizeof(esp_image_header_t)) {
    ESP_LOGE(TAG, "Image too small: %u bytes (minimum: %u bytes)", length,
  ...
  if (header->magic != ESP_IMAGE_HEADER_MAGIC) {
    ESP_LOGE(TAG, "Invalid image magic number: 0x%02X (expected: 0xE9)",
             header->magic);
  ...
  if (length > partition->size) {
    ESP_LOGE(TAG,
             "Firmware size (%u bytes) exceeds partition capacity (%lu bytes)",
             length, partition->size);
    return ESP_ERR_INVALID_SIZE;
  }
  ESP_LOGI(TAG, "✓ Firmware size valid (%u / %lu bytes)", length,
           partition->size);

  // 5. Validate segment count
  if (header->segment_count > ESP_IMAGE_MAX_SEGMENTS) {
    ESP_LOGE(TAG, "Too many segments: %d (max: %d)", header->segment_count,
...
esp_err_t ota_begin_update(size_t image_size) {
  ...
  strncpy(g_ota_ctx.status_message, "Preparing to write firmware",
...
esp_err_t ota_write_chunk(const uint8_t *data, size_t length) {
  ...
  if (!g_ota_ctx.header_validated && g_ota_ctx.written_size == 0) {
    ...
    if (length >= sizeof(esp_image_header_t)) {
      ...
      // Validate chip type
      if (header->chip_id != ESP_CHIP_ID_ESP32S3) {
        ESP_LOGE(TAG, "Incompatible chip type: %d (expected ESP32-S3)",
                 header->chip_id);
        snprintf(g_ota_ctx.error_message, sizeof(g_ota_ctx.error_message),
                 "Firmware not compatible with ESP32-S3 (chip_id: %d)",
                 header->chip_id);
        g_ota_ctx.state = OTA_STATE_ERROR;
        ota_abort_update();
        return ESP_ERR_NOT_SUPPORTED;
      ...
      if (header->segment_count > ESP_IMAGE_MAX_SEGMENTS) {
        ...
        snprintf(g_ota_ctx.error_message, sizeof(g_ota_ctx.error_message),
                 "Invalid firmware image structure (segments: %d)",
      ...
      // SPI mode: 0-5, SPI speed is 4-bit (always valid), Flash size: check
      // against max
      if (header->spi_mode > 5 ||
          header->spi_size >= ESP_IMAGE_FLASH_SIZE_MAX) {
        ESP_LOGE(TAG, "Invalid SPI configuration in image header");
        snprintf(g_ota_ctx.error_message, sizeof(g_ota_ctx.error_message),
                 "Invalid firmware image configuration (SPI mode: %d)",
                 header->spi_mode);
        g_ota_ctx.state = OTA_STATE_ERROR;
        ota_abort_update();
        return ESP_ERR_IMAGE_INVALID;
      ...
      ESP_LOGI(TAG, "  Magic: 0xE9, Chip: ESP32-S3, Segments: %d",
               header->segment_count);
      g_ota_ctx.header_validated = true;
    } else {
      ESP_LOGW(TAG,
               "First chunk too small for header validation (%u bytes), will "
  ...
  size_t bytes_since_report =
      g_ota_ctx.written_size - g_ota_ctx.last_report_bytes;
  bool should_report = false;

  // Report if 10% progress change
  if (new_percent != g_ota_ctx.progress_percent &&
      (new_percent % 10 == 0 || new_percent == 100)) {
    should_report = true;
  }
  ...
  if (should_report) {
    ...
    snprintf(g_ota_ctx.status_message, sizeof(g_ota_ctx.status_message),
             ...
             g_ota_ctx.progress_percent, g_ota_ctx.written_size,
...
esp_err_t ota_end_update(void) {
  ...
  strncpy(g_ota_ctx.status_message, "Verifying firmware integrity",
          sizeof(g_ota_ctx.status_message) - 1);
  ...
  strncpy(g_ota_ctx.status_message, "Applying firmware update",
          sizeof(g_ota_ctx.status_message) - 1);
  ...
  strncpy(g_ota_ctx.status_message, "Complete: Firmware ready to apply",
          sizeof(g_ota_ctx.status_message) - 1);

  uint32_t elapsed =
      (uint32_t)(esp_timer_get_time() / 1000000) - g_ota_ctx.start_time;
...
esp_err_t ota_rollback(void) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to set boot partition for rollback: %s",
             esp_err_to_name(err));
    return err;
  }
...
const char *ota_get_status_string(void) {
  ...
  case OTA_STATE_ERROR:
    return g_ota_ctx.error_message;
  default:
    return "Unknown";
  }
}
