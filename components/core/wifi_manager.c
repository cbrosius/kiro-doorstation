...
// Parameters passed to the credential test task - must be heap-allocated.
typedef struct {
  char *ssid;
  char *password;
} credential_test_params_t;

// Forward declarations
static void wifi_clear_scan_results(void);
static void credential_test_task_func(void *pvParameters);

void wifi_manager_register_ap_start_callback(
    wifi_manager_ap_start_callback_t cb) {
  ap_start_cb = cb;
}
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  ESP_LOGI(TAG, "WiFi event: base=%s, id=%d",
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    ESP_LOGW(TAG, "WiFi disconnected (reason: %d - %s), retry %d/%d",
             disconnected->reason, wifi_reason_to_string(disconnected->reason),
    ...
    if (current_mode == WIFI_MODE_STA) {
      ...
      ESP_LOGI(TAG, "IP obtained in APSTA mode - keeping captive portal active "
    ...
    if (ap_count > 0) {
      ...
      if (ap_list != NULL) {
        ...
        if (err == ESP_OK) {
          ...
          for (int i = 0; i < ap_count && scan_results_count < MAX_SCAN_RESULTS;
               i++) {
            ...
            // Store result
            strncpy(scan_results[scan_results_count].ssid,
                    (char *)ap_list[i].ssid, WIFI_SSID_MAX_LEN - 1);
            scan_results[scan_results_count].ssid[WIFI_SSID_MAX_LEN - 1] = '\0';
            scan_results[scan_results_count].rssi = ap_list[i].rssi;
            scan_results[scan_results_count].secure =
            ...
            ESP_LOGI(TAG, "Stored network: %s (RSSI: %d, Secure: %s)",
                     ...
                     scan_results[scan_results_count - 1].secure ? "yes"
                                                                 : "no");
          }
    ...
    ESP_LOGI(TAG, "ESPHome approach: Scan completed, STA interface ready for "
                  "future scans");
  }
}
...
void wifi_manager_init(void) {
  ...
  cfg.ampdu_rx_enable =
      0; // Disable AMPDU RX to prevent ADDBA/DELBA timer issues
  ...
  if (saved_config.configured) {
    ESP_LOGI(TAG, "Saved WiFi config found, attempting STA connection: %s",
             saved_config.ssid);
    ...
    if (wifi_is_connected()) {
      ESP_LOGI(TAG,
               "Successfully connected to saved WiFi, staying in STA mode");
      return; // Exit early, no captive portal needed
    } else {
      ESP_LOGW(TAG,
               "Failed to connect to saved WiFi, falling back to APSTA mode");
    }
  } else {
    ESP_LOGI(
        TAG,
        "No saved WiFi config found, starting APSTA mode for captive portal");
  }

  // Start APSTA mode for captive portal functionality (fallback or initial
  // setup)
...
void wifi_clear_config(void) {
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "Failed to open NVS for clearing config: %s",
             esp_err_to_name(err));
  }
}
...
void wifi_start_background_scan(void) {
  ...
  ESP_LOGI(
      TAG,
      "ESPHome approach: Starting scan with pre-configured STA interface...");
...
int wifi_get_scan_results(wifi_scan_result_t *results, int max_results) {
  ...
  int count =
      (scan_results_count < max_results) ? scan_results_count : max_results;
...
int wifi_scan_networks(wifi_scan_result_t **results) {
  ...
  ESP_LOGI(TAG, "Scan completed with %d results after %d ms",
  ...
  memcpy(scan_results_ptr, scan_results,
         sizeof(wifi_scan_result_t) * scan_results_count);
 ...
 */
static void credential_test_event_handler(void *arg,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED) {
    ...
    ip_event_got_ip_t *event = (ip_event_got_ip_t *)event_data;
    ESP_LOGI(TAG, "Credential test: IP obtained: " IPSTR,
             IP2STR(&event->ip_info.ip));
...
static void credential_test_task_func(void *pvParameters) {
  // Unpack heap-allocated params and free immediately so ownership is clear.
  credential_test_params_t *params = (credential_test_params_t *)pvParameters;
  char *ssid = params->ssid;
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to start credential test connection: %s",
             esp_err_to_name(err));
    goto cleanup;
  }
  ...
  EventBits_t bits = xEventGroupWaitBits(
      test_event_group, TEST_CONNECTED_BIT | TEST_FAILED_BIT, pdFALSE, pdFALSE,
  ...
  if (bits & TEST_CONNECTED_BIT) {
    ...
    EventBits_t ip_bits =
        xEventGroupWaitBits(test_event_group, TEST_IP_BIT, pdFALSE, pdFALSE,
    ...
    if (ip_bits & TEST_IP_BIT) {
      ...
      esp_netif_ip_info_t ip_info;
      if (esp_netif_get_ip_info(sta_netif, &ip_info) == ESP_OK) {
        snprintf(tested_sta_ip, sizeof(tested_sta_ip), IPSTR,
                 IP2STR(&ip_info.ip));
      ...
      ESP_LOGW(TAG,
               "Credential test: Connected but no IP assigned within timeout");
      memset(tested_sta_ip, 0, sizeof(tested_sta_ip));
    }
  } else {
    ESP_LOGW(TAG,
             "Credential test failed or timed out - no connection established");
    memset(tested_sta_ip, 0, sizeof(tested_sta_ip));
  }
...
bool wifi_test_credentials(const char *ssid, const char *password) {
  ...
  // Allocate a struct on the heap so the task can safely read it after this
  // function returns (a stack-based array would be a race condition).
  credential_test_params_t *params = malloc(sizeof(credential_test_params_t));
  if (!params) {
    ESP_LOGE(TAG, "Failed to allocate memory for credential test params");
    return false;
  }

  params->ssid = strdup(ssid);
  params->password = strdup(password ? password : "");

  if (!params->ssid || !params->password) {
    ESP_LOGE(TAG, "Failed to allocate memory for credential test strings");
    free(params->ssid);
  ...
  // Create task for parallel testing. Ownership of params transfers to the
  // task.
...
/**
 * @brief Transition from APSTA mode to STA-only mode after successful
 ...
 */
bool wifi_is_testing_credentials(void) { return is_testing_credentials; }
