...
static void doorbell_task(void *arg) {
  ...
  static TickType_t last_call_time[2] = {0,
                                         0}; // Debounce tracking for each bell
  ...
  while (1) {
    if (xQueueReceive(doorbell_queue, &event, portMAX_DELAY)) {
      ...
      if (event.bell == DOORBELL_1) {
        ...
        hw_status_log_event(HW_EVENT_BELL_PRESS, 1, "Physical Button");
        ...
        hw_status_log_event(HW_EVENT_BELL_PRESS, 2, "Physical Button");
...
void gpio_handler_init(void) {
  ...
  xTaskCreate(doorbell_task, "doorbell_task", 8192, NULL, 5,
              &doorbell_task_handle);

  // Configure doorbell inputs
  gpio_config_t doorbell_config = {.pin_bit_mask = (1ULL << DOORBELL_1_PIN) |
                                                   (1ULL << DOORBELL_2_PIN),
  ...
  gpio_config_t boot_button_config = {
      ...
      .pull_up_en = GPIO_PULLUP_ENABLE,
      .pull_down_en = GPIO_PULLDOWN_DISABLE,
      .intr_type = GPIO_INTR_DISABLE // No interrupt, will be polled by reset
                                     // monitor task
  };
  gpio_config(&boot_button_config);

  // Configure relay outputs
  gpio_config_t relay_config = {.pin_bit_mask = (1ULL << DOOR_RELAY_PIN) |
                                                (1ULL << LIGHT_RELAY_PIN),
  ...
  gpio_isr_handler_add(DOORBELL_1_PIN, doorbell_isr_handler,
                       (void *)DOORBELL_1);
  gpio_isr_handler_add(DOORBELL_2_PIN, doorbell_isr_handler,
                       (void *)DOORBELL_2);
...
static void reset_monitor_task(void *arg) {
  ...
  const uint32_t required_presses =
      RESET_BUTTON_HOLD_TIME_MS / 100;       // 100 checks for 10 seconds
  ...
  while (1) {
    ...
    if (is_pressed) {
      ...
      if (press_count >= short_press_threshold && press_count % 10 == 0) {
        ...
        ESP_LOGI(
            TAG,
            "BOOT button held for %lu seconds (hold 10s to reset password)...",
            seconds_held);
      }
      ...
      if (press_count >= required_presses) {
        ...
        if (err == ESP_OK) {
          ...
          ESP_LOGW(TAG,
                   "Initial setup wizard will be triggered on next web access");
        } else {
      ...
      // Button released
      if (was_pressed && press_count > 0 &&
          press_count < short_press_threshold) {
        // Short press detected - trigger doorbell call
        ESP_LOGI(TAG, "BOOT button short press - triggering doorbell call");
        hw_status_log_event(HW_EVENT_BELL_PRESS, 1, "BOOT Button (Simulated)");
        const char *target1 = sip_get_target1();
        if (target1 && strlen(target1) > 0) {
          sip_client_make_call(target1);
        } else {
...
void gpio_start_reset_monitor(void) {
  ...
  BaseType_t result = xTaskCreate(reset_monitor_task, "reset_monitor",
                                  8192, // Increased stack for SIP operations
                                  NULL, 5, &reset_monitor_task_handle);

  if (result == pdPASS) {
    ESP_LOGI(TAG, "Reset monitor task created - short press for doorbell, hold "
                  "10s to reset password");
  } else {
