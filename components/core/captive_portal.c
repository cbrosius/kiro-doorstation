...
extern const uint8_t
    captive_setup_html_start[] asm("_binary_captive_setup_html_start");
extern const uint8_t
    captive_setup_html_end[] asm("_binary_captive_setup_html_end");

/**
 * @brief Handler for captive portal setup page
 */
static esp_err_t captive_setup_handler(httpd_req_t *req) {
  ESP_LOGI(TAG, "HTTP REQUEST: Serving captive setup page for URI: %s",
           req->uri);

  httpd_resp_set_type(req, "text/html");
  const size_t captive_setup_html_size =
      (uintptr_t)captive_setup_html_end - (uintptr_t)captive_setup_html_start;
  httpd_resp_send(req, (const char *)captive_setup_html_start,
                  captive_setup_html_size);
  return ESP_OK;
}
...
static esp_err_t captive_hotspot_detect_handler(httpd_req_t *req) {
  ...
  const char *redirect_html =
      ...
      "content=\"0;url=/setup.html\"></head><body></body></html>";
...
static esp_err_t captive_redirect_handler(httpd_req_t *req) {
  ESP_LOGI(
      TAG,
      "HTTP REQUEST: Unknown captive URL: %s - checking for STA IP redirect",
      req->uri);
  ...
  if (sta_ip && strlen(sta_ip) > 0) {
    ...
    // After successful redirect, stop captive portal and transition to STA-only
    // mode
    ESP_LOGI(TAG, "Successful redirect completed, stopping captive portal and "
                  "transitioning to STA-only mode");
    captive_portal_stop();
    dns_responder_stop();
...
static esp_err_t captive_post_wifi_config_handler(httpd_req_t *req) {
  ...
  httpd_resp_set_type(req, "application/json");
  httpd_resp_send(
      req, "{\"status\":\"success\",\"message\":\"WiFi configuration saved\"}",
      strlen(
          "{\"status\":\"success\",\"message\":\"WiFi configuration saved\"}"));
  return ESP_OK;
}
...
static esp_err_t captive_post_wifi_connect_handler(httpd_req_t *req) {
  ...
  if (wifi_test_credentials(ssid->valuestring, pwd)) {
    ...
    httpd_resp_send(
        req,
        "{\"status\":\"testing\",\"message\":\"Testing WiFi credentials...\"}",
        strlen("{\"status\":\"testing\",\"message\":\"Testing WiFi "
               "credentials...\"}"));
  } else {
    ESP_LOGE(
        TAG,
        "Failed to start credential testing, staying in APSTA mode for retry");
    cJSON_Delete(root);
    ...
    httpd_resp_send(req,
                    ...
                    "credential testing\"}",
                    strlen("{\"status\":\"error\",\"message\":\"Failed to "
...
bool captive_portal_start(void) {
  ...
  ESP_LOGI(TAG,
           "HTTP server starting - binding to all interfaces in APSTA mode");

  esp_err_t err = httpd_start(&captive_server, &config);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to start captive portal server: %s",
             esp_err_to_name(err));
    return false;
  }

  ESP_LOGI(TAG, "Captive portal HTTP server started on port %d, handle: %p", 80,
           captive_server);
  ...
  httpd_uri_t setup_uri = {.uri = "/setup.html",
                           .method = HTTP_GET,
  ...
  httpd_uri_t msft_connect_uri = {.uri = "/connecttest.txt",
                                  .method = HTTP_GET,
  ...
  // iOS/macOS variants
  httpd_uri_t hotspot_detect_alt_uri = {.uri = "/hotspotdetect.html",
                                        .method = HTTP_GET,
                                        .handler =
                                            captive_hotspot_detect_handler,
                                        .user_ctx = NULL};
  ...
  httpd_uri_t msft_redirect_uri = {.uri = "/redirect",
                                   .method = HTTP_GET,
                                   .handler = captive_msftconnecttest_handler,
                                   .user_ctx = NULL};
  ...
  httpd_uri_t success_txt_uri = {.uri = "/success.txt",
                                 .method = HTTP_GET,
                                 .handler = captive_ncsi_handler,
                                 .user_ctx = NULL};
  ...
  if (reg_err != ESP_OK)
    ESP_LOGE(TAG, "Failed to register catch-all handler: %s",
             esp_err_to_name(reg_err));
  ...
  // NOTE: This is a duplicate registration of /generate_204 - removing to avoid
  // warning httpd_uri_t gstatic_generate_204_uri = {
  //     .uri = "/generate_204",
  //     .method = HTTP_GET,
  //     .handler = captive_generate_204_handler,
  //     .user_ctx = NULL
...
void captive_api_register_handlers(httpd_handle_t server) {
  ...
  httpd_uri_t wifi_config_get_uri = {.uri = "/api/wifi/config",
                                     .method = HTTP_GET,
                                     .handler = captive_get_wifi_config_handler,
                                     .user_ctx = NULL};

  // WiFi config POST handler
  httpd_uri_t wifi_config_post_uri = {.uri = "/api/wifi/config",
                                      .method = HTTP_POST,
                                      .handler =
                                          captive_post_wifi_config_handler,
                                      .user_ctx = NULL};
