...
static uint32_t last_message_timestamp =
    0; // Track when we last sent a SIP message
...
static const char *state_names[] = {
    "IDLE",    "REGISTERING", "REGISTERED",    "CALLING",
    "RINGING", "CONNECTED",   "DTMF_SENDING",  "DISCONNECTED",
    "ERROR",   "AUTH_FAILED", "NETWORK_ERROR", "TIMEOUT"};
...
// Store Call-ID and From tag from initial REGISTER for reuse in authenticated
// REGISTER
...
void sip_add_log_entry(const char *type, const char *message) {
  ...
  if (strcmp(type, "error") == 0) {
    ...
    NTP_LOGI(TAG, "[%s] %s", type,
             message); // Changed from LOGD to LOGI for visibility
  } else {
...
void sip_set_config(const sip_config_t *config) {
  if (config) {
    sip_config = *config;
...
void sip_save_config_from_struct(const sip_config_t *config) {
  if (config) {
    sip_save_config(config->server, config->username, config->password,
                    config->apartment1_uri, config->apartment2_uri,
                    config->port);
  }
}
...
static void sip_task(void *pvParameters __attribute__((unused))) {
  ...
  while (1) {
    ...
    if (reinit_requested) {
      ...
      if (sip_config.configured) {
        ...
        if (sip_socket >= 0) {
          ...
          if (bind(sip_socket, (struct sockaddr *)&local_addr,
                   sizeof(local_addr)) == 0) {
    ...
    if (current_state == SIP_STATE_CONNECTED &&
      ...
      uint32_t elapsed_rtp = xTaskGetTickCount() * portTICK_PERIOD_MS -
                             last_rtp_received_timestamp;
      if (elapsed_rtp >= rtp_timeout_ms) {
        led_handler_set_state(LED_STATE_ERROR);
        sip_add_log_entry(
            "error",
            "RTP timeout - no audio received for 5 seconds. Hanging up.");
        sip_client_hangup();
    ...
    if (last_message_timestamp > 0 && sip_socket >= 0) {
      uint32_t elapsed =
          xTaskGetTickCount() * portTICK_PERIOD_MS - last_message_timestamp;
      if (elapsed >= sip_response_timeout_ms) {
        sip_add_log_entry(
            "error",
            "SIP response timeout - no response from server for 3 seconds");
    ...
    if (last_connection_retry_timestamp > 0) {
      ...
      if (elapsed >= connection_retry_delay_ms) {
        ...
        snprintf(retry_debug, sizeof(retry_debug),
                 "Retrying SIP connection: current_state=%s, socket=%d, "
                 "configured=%d",
        ...
        if (sip_socket < 0) {
          ...
          if (sip_socket >= 0) {
            ...
            if (bind(sip_socket, (struct sockaddr *)&local_addr,
              ...
              snprintf(bind_msg, sizeof(bind_msg),
                       "Socket recreated (fd=%d), changing state to IDLE",
                       sip_socket);
              ...
              init_timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS;
              sip_add_log_entry("info", "Auto-registration timestamp set - "
                                        "will trigger in next loop iteration");
            } else {
              sip_add_log_entry(
                  "error",
                  "Failed to bind socket after timeout - will retry later");
              close(sip_socket);
              sip_socket = -1;
              // Reschedule retry
              last_connection_retry_timestamp =
            ...
            sip_add_log_entry(
                "error",
                "Failed to create socket after timeout - will retry later");
            // Reschedule retry
            last_connection_retry_timestamp =
                xTaskGetTickCount() * portTICK_PERIOD_MS;
          }
          ...
          snprintf(socket_msg, sizeof(socket_msg),
                   "Socket already exists (fd=%d) during retry - closing and "
                   "recreating",
                   sip_socket);
    ...
    if (init_timestamp > 0 && current_state == SIP_STATE_IDLE &&
      ...
      snprintf(
          auto_reg_msg, sizeof(auto_reg_msg),
          "Auto-registration triggered: state=%s, socket=%d, configured=%d",
          state_names[current_state], sip_socket,
          sip_config.configured ? 1 : 0);
    ...
    if (registration_requested && current_state != SIP_STATE_REGISTERED) {
      ...
      if (sip_socket < 0) {
        sip_add_log_entry("info",
                          "Socket closed - recreating before registration");
        ...
        sip_add_log_entry(
            "info", "Socket recreated and bound - ready for registration");
      } else {
        char socket_ok_msg[128];
        snprintf(socket_ok_msg, sizeof(socket_ok_msg),
                 "Socket already exists (fd=%d) - proceeding with registration",
                 sip_socket);
    ...
    if (sip_socket >= 0) {
      ...
      if (len > 0) {
        ...
        snprintf(state_log, sizeof(state_log),
                 "Processing message in state: %s", state_names[current_state]);
        ...
        if (strstr(buffer, "SIP/2.0 200 OK")) {
          if (current_state == SIP_STATE_REGISTERING) {
            ...
            if (sdp_start) {
              ...
              if (sdp_end) {
                ...
                char sdp_log[512];
                snprintf(sdp_log, sizeof(sdp_log), "Remote SDP: %.*s",
                         (int)sdp_len, sdp_start);
            ...
            char ip_log[128];
            snprintf(ip_log, sizeof(ip_log),
                     "RTP remote IP parsed from SDP: %s (port: %d)", remote_ip,
                     remote_rtp_port);
          ...
          if (current_state == SIP_STATE_REGISTERING) {
            ...
            snprintf(
                ...
                "Authentication required (attempt %d/%d), parsing challenge",
            ...
            if (auth_attempt_count > MAX_AUTH_ATTEMPTS) {
              sip_add_log_entry("error", "Max authentication attempts exceeded "
            ...
            if (via_header) {
              if (extract_received_ip(via_header, public_ip,
                                      sizeof(public_ip))) {
                char log_msg[128];
                snprintf(log_msg, sizeof(log_msg),
                         "Public IP extracted from 401: %s", public_ip);
                sip_add_log_entry("info", log_msg);
              } else {
            ...
            // Per RFC 3261, a UAC must send an ACK for all final responses to
            // an INVITE, including a 401. This stops the server from
            // retransmitting the 401 challenge.
            send_ack_for_error_response(buffer);

            // CRITICAL: Check if this 401 is for our authenticated INVITE or a
            // retransmission initial_invite_branch = first INVITE without auth
            // auth_invite_branch = authenticated INVITE (only set when retry
            // happens)

            const char *via_ptr = strstr(buffer, "Via:");

            if (via_ptr && has_invite_auth_challenge &&
                auth_invite_branch != 0) {
              // We've sent an authenticated INVITE - check if this 401 is for
              // it
              ...
              if (branch_param) {
                ...
                snprintf(branch_log, sizeof(branch_log),
                         ...
                         received_branch, initial_invite_branch,
                         auth_invite_branch);
                ...
                if (received_branch == initial_invite_branch &&
                  ...
                  sip_add_log_entry(
                      "info",
                      "401 is retransmission of initial challenge - ignoring");
                  continue;
                }
              }
            }
            ...
            if (invite_auth_attempt_count >= MAX_INVITE_AUTH_ATTEMPTS) {
              ...
              snprintf(err_msg, sizeof(err_msg),
                       "Max INVITE auth attempts (%d) exceeded - giving up on "
                       "this call",
              ...
              snprintf(fail_debug, sizeof(fail_debug),
                       "INVITE auth failed - Last attempt used: nonce=%s, "
            ...
            if (new_challenge.valid) {
              ...
              // Only reset counter if this is a genuinely NEW nonce (different
              // from previous)
              if (!has_invite_auth_challenge ||
                  strcmp(invite_auth_challenge.nonce, new_challenge.nonce) !=
                      0) {
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg),
                         "New INVITE auth challenge (nonce changed) - "
              ...
              sip_add_log_entry("info", "INVITE authentication challenge "
                                        "updated - will retry with auth");
                ...
                if (uri_start) {
                  ...
                  if (uri_end) {
                    ...
                    if (uri_len < sizeof(retry_uri)) {
                      ...
                      char retry_log[256];
                      snprintf(
                          retry_log, sizeof(retry_log),
                          "Retrying INVITE with auth (attempt %d/%d) to %s",
                          invite_auth_attempt_count + 1,
                          MAX_INVITE_AUTH_ATTEMPTS, retry_uri);
              ...
              sip_add_log_entry("error",
                                "Failed to parse INVITE auth challenge");
              current_state = SIP_STATE_REGISTERED;
              call_start_timestamp = 0;
              has_invite_auth_challenge = false;
              invite_auth_attempt_count = 0;
          ...
          if (current_state == SIP_STATE_REGISTERING) {
            sip_add_log_entry("error",
                              "500 during REGISTER - clearing auth state");
            auth_attempt_count = 0;
            has_initial_transaction_ids = false;
            current_state = SIP_STATE_DISCONNECTED;
          } else if (current_state == SIP_STATE_CALLING ||
          ...
          sip_add_log_entry(
              "info",
              "Connection retry scheduled in 10 seconds after 500 error");
          ...
          sip_add_log_entry(
              "info",
              "Connection retry scheduled in 10 seconds after 503 error");
          ...
          if (headers.valid) {
            ...
            // Check if this is a retransmission (same Call-ID and branch within
            // 10 seconds)
            ...
            if (strlen(last_error_call_id) > 0 &&
              ...
              sip_add_log_entry(
                  "info",
                  "603 Decline is a retransmission - ignoring to prevent loop");
            } else {
              ...
              strncpy(last_error_call_id, headers.call_id,
                      sizeof(last_error_call_id) - 1);
              strncpy(last_error_via_branch, via_branch,
                      sizeof(last_error_via_branch) - 1);
              last_error_timestamp = current_time;

              // Send ACK to stop retransmissions (RFC 3261 requirement)
              send_ack_for_error_response(buffer);

              // Clear INVITE authentication state to prevent retransmission
              // loops
              has_invite_auth_challenge = false;
              memset(&invite_auth_challenge, 0, sizeof(invite_auth_challenge));
              invite_auth_attempt_count = 0;
              ...
              sip_add_log_entry(
                  "info",
                  "INVITE authentication state cleared - ready for new call");
            }
          } else {
          ...
          if (content_type && body_start) {
            ...
            if (strstr(content_type, "application/dtmf-relay") ||
              ...
              if (signal_line) {
                ...
                if ((dtmf_signal >= '0' && dtmf_signal <= '9') ||
                  ...
                  if (current_state == SIP_STATE_CONNECTED) {
                    ...
                    case 'd':
                      ...
                      break;
                    default:
                      sip_add_log_entry("warning",
                                        "Invalid DTMF signal in INFO packet");
                    ...
                    snprintf(state_warning, sizeof(state_warning),
                             "DTMF INFO received but not in CONNECTED state "
                             "(state=%s)",
                             (current_state <
                              sizeof(state_names) / sizeof(state_names[0]))
                                 ? state_names[current_state]
                ...
                sip_add_log_entry(
                    "info", "INFO with DTMF content-type but no Signal line");
              }
            } else {
          ...
          char state_log[128];
          snprintf(state_log, sizeof(state_log),
                   "Processing INVITE in state: %s", current_state_name);
          sip_add_log_entry("info", state_log);
          ...
          if (current_state != SIP_STATE_IDLE &&
              current_state != SIP_STATE_REGISTERED) {
            char busy_msg[128];
            snprintf(busy_msg, sizeof(busy_msg),
                     "Busy - cannot accept call (state: %s)",
                     current_state_name);
          ...
          // Check for Require header early - reject if it requires unsupported
          // extensions RFC 3261 §20.32: If we don't support a required
          // extension, send 420 Bad Extension
          const char *require_hdr = strstr(buffer, "Require:");
          if (require_hdr) {
            // Extract required extensions
            require_hdr += 8; // Skip "Require:"
            while (*require_hdr == ' ')
              require_hdr++; // Skip whitespace
            ...
            if (require_end) {
              ...
              if (len < sizeof(required_ext)) {
                ...
                if (headers.valid) {
                  ...
                  send_sip_response(420, "Bad Extension", &headers,
                                    unsupported_hdr, NULL);

                  sip_add_log_entry("info",
                                    "420 Bad Extension sent - INVITE rejected");
                } else {
                  sip_add_log_entry("error",
                                    "Failed to parse headers for 420 response");
                }
          ...
          if (from_ptr) {
            ...
            // Handle RFC 2976 INFO method for DTMF relay (secure alternative to
            // audio tones)
            ...
            const char *content_type = strstr(buffer, "Content-Type:");
            if (content_type &&
                strstr(content_type, "application/dtmf-relay")) {
              ...
              if (body_start) {
                ...
                char dtmf_log[128];
                snprintf(dtmf_log, sizeof(dtmf_log),
                         "DTMF relay: Signal=%s, Duration=%s", signal,
                         duration);
                sip_add_log_entry("info", dtmf_log);
                ...
                if (strlen(signal) == 1) {
                  ...
                  switch (dtmf_char) {
                  case '0':
                    event_code = DTMF_EVENT_0;
                    break;
                  case '1':
                    event_code = DTMF_EVENT_1;
                    break;
                  case '2':
                    event_code = DTMF_EVENT_2;
                    break;
                  case '3':
                    event_code = DTMF_EVENT_3;
                    break;
                  case '4':
                    event_code = DTMF_EVENT_4;
                    break;
                  case '5':
                    event_code = DTMF_EVENT_5;
                    break;
                  case '6':
                    event_code = DTMF_EVENT_6;
                    break;
                  case '7':
                    event_code = DTMF_EVENT_7;
                    break;
                  case '8':
                    event_code = DTMF_EVENT_8;
                    break;
                  case '9':
                    event_code = DTMF_EVENT_9;
                    break;
                  case '*':
                    event_code = DTMF_EVENT_STAR;
                    break;
                  case '#':
                    event_code = DTMF_EVENT_HASH;
                    break;
                  case 'A':
                  case 'a':
                    event_code = DTMF_EVENT_A;
                    break;
                  case 'B':
                  case 'b':
                    event_code = DTMF_EVENT_B;
                    break;
                  case 'C':
                  case 'c':
                    event_code = DTMF_EVENT_C;
                    break;
                  case 'D':
                  case 'd':
                    event_code = DTMF_EVENT_D;
                    break;
                  default:
                    ESP_LOGW(TAG, "Invalid DTMF character in INFO: %c",
                             dtmf_char);
                    break;
                  }

                  if (event_code <= DTMF_EVENT_D) {
                    // Process via RFC 4733 handler (end bit set for INFO
                    // method)
                    dtmf_process_telephone_event(event_code);
                    sip_add_log_entry(
                        "info", "DTMF processed via RFC 2976 INFO method");
                  }
                }
              }
              ...
              sip_add_log_entry(
                  "info", "INFO method not supported for non-DTMF purposes");
              char allow_header[128];
              snprintf(allow_header, sizeof(allow_header),
                       "Allow: INVITE, ACK, BYE, CANCEL, OPTIONS\r\n"
                       "Accept: application/dtmf-relay\r\n");
              send_sip_response(501, "Not Implemented", &headers, allow_header,
                                NULL);
            }
            ...
            char log_msg[128];
            snprintf(log_msg, sizeof(log_msg),
                     "%s method not implemented - sending 501", method);
            sip_add_log_entry("info", log_msg);
            ...
            if (!headers.valid) {
              char err_msg[128];
              snprintf(err_msg, sizeof(err_msg), "Failed to parse %s headers",
                       method);
              sip_add_log_entry("error", err_msg);
              continue;
            }
            ...
            snprintf(log_msg, sizeof(log_msg),
                     "%s not implemented - 501 sent with supported methods",
                     method);
            sip_add_log_entry("info", log_msg);
          ...
          snprintf(response, sizeof(response),
                   ...
                   "Content-Length: %d\r\n\r\n%s",
                   via_header, from_header, to_with_tag, call_id, cseq_num,
                   sip_config.username, local_ip, strlen(sdp), sdp);
          ...
          if (resolve_hostname(sip_config.server, &server_addr,
                               (uint16_t)sip_config.port)) {
            int sent =
                sendto(sip_socket, response, strlen(response), 0,
                       (struct sockaddr *)&server_addr, sizeof(server_addr));
            if (sent > 0) {
              ...
              snprintf(rtp_log, sizeof(rtp_log),
                       "Starting RTP session to %s:5004", remote_ip);
              ...
              led_handler_set_state(LED_STATE_CALL_ACTIVE);
              sip_add_log_entry("info",
                                "Incoming call answered - State: CONNECTED");
            ...
            sip_add_log_entry("error", "DNS lookup failed");
                   ...
                   strncmp(buffer, "BYE ", 4) == 0) {
          // Check for BYE request (not response)
          sip_add_log_entry(
              "info", "BYE message detected - processing call termination");
          ...
          snprintf(bye_response, sizeof(bye_response),
                   ...
                   "CSeq: %d BYE\r\n"
                   "Content-Length: 0\r\n\r\n",
                   local_ip, sip_config.username, sip_config.server,
                   sip_config.username, sip_config.server, call_id, cseq_num);
...
void sip_client_init(void) {
  ...
  if (sip_config.configured) {
    ...
    if (bind(sip_socket, (struct sockaddr *)&local_addr, sizeof(local_addr)) <
        0) {
      ESP_LOGE(TAG, "Error binding SIP socket to port 5060");
      close(sip_socket);
      sip_socket = -1;
    ...
    init_timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS;
    ESP_LOGI(TAG, "SIP client ready. Auto-registration will start in %lu ms",
             auto_register_delay_ms);
...
bool sip_client_register(void) {
  ...
  char log_msg[256];
  snprintf(log_msg, sizeof(log_msg),
           "Initial REGISTER: Call-ID=%s, From-tag=%s", initial_call_id,
           initial_from_tag);
  sip_add_log_entry("info", log_msg);
  ...
  snprintf(register_msg, sizeof(register_msg),
           ...
           "Content-Length: 0\r\n\r\n",
           sip_config.server, local_ip, branch_id, sip_config.username,
           sip_config.server, initial_from_tag, sip_config.username,
           sip_config.server, initial_call_id, sip_config.username, contact_ip);
  ...
  if (sent < 0) {
    char err_msg[64];
    snprintf(err_msg, sizeof(err_msg), "Error sending REGISTER message: %d",
             sent);
...
static bool sip_client_register_auth(sip_auth_challenge_t *challenge) {
  if (!sip_config.configured || sip_socket < 0 || !challenge ||
      !challenge->valid) {
    current_state = SIP_STATE_ERROR;
    return false;
  }
  ...
  if (!has_initial_transaction_ids) {
    sip_add_log_entry(
        "error", "No initial transaction IDs stored - cannot authenticate");
    current_state = SIP_STATE_ERROR;
    return false;
  }
  ...
  if (nc_value > 99999999)
    nc_value = 1; // Prevent overflow
  ...
  snprintf(debug_msg, sizeof(debug_msg),
           "REGISTER Auth attempt %d: nonce='%s', nc='%s', cnonce='%s'",
  ...
  calculate_digest_response(sip_config.username, sip_config.password,
                            challenge->realm, challenge->nonce, "REGISTER",
                            register_uri, challenge->qop, nc_str, cnonce,
                            response);

  // Log the calculated response for debugging
  char response_debug[128];
  snprintf(response_debug, sizeof(response_debug),
           "REGISTER digest response: %s", response);
  sip_add_log_entry("info", response_debug);

  // Log digest calculation for debugging
  snprintf(log_msg, sizeof(log_msg), "Digest calculated: response=%s",
           response);
  sip_add_log_entry("info", log_msg);
  ...
  char ip_log[128];
  snprintf(ip_log, sizeof(ip_log),
           "Using contact IP: %s (public: %s, local: %s)", contact_ip,
           public_ip, local_ip);
  sip_add_log_entry("info", ip_log);
  ...
  int len = snprintf(register_msg, sizeof(register_msg),
                     ...
                     "username=\"%s\",realm=\"%s\",nonce=\"%s\",uri=\"sip:%s\","
                     "response=\"%s\"",
                     sip_config.server, contact_ip, branch, sip_config.username,
                     sip_config.server, initial_from_tag, // REUSE stored tag
                     sip_config.username, sip_config.server,
                     initial_call_id, // REUSE stored Call-ID
                     sip_config.username,
                     contact_ip, // Use public IP for Contact header
                     sip_config.username, challenge->realm, challenge->nonce,
                     sip_config.server, response);

  // Add qop parameters if present (NO spaces after commas)
  if (strlen(challenge->qop) > 0) {
    len +=
        snprintf(register_msg + len, sizeof(register_msg) - len,
  ...
  if (strlen(challenge->algorithm) > 0 &&
      strcmp(challenge->algorithm, "MD5") != 0) {
    len += snprintf(register_msg + len, sizeof(register_msg) - len,
  ...
  last_message_timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS;
  snprintf(log_msg, sizeof(log_msg), "Authenticated REGISTER sent (%d bytes)",
           sent);
  sip_add_log_entry("info", log_msg);
  return true;
}
...
void sip_client_make_call(const char *uri) {
  // Allow calls only when IDLE or REGISTERED, or when retrying with
  // authentication
  if (current_state != SIP_STATE_IDLE &&
      current_state != SIP_STATE_REGISTERED &&
      !(has_invite_auth_challenge &&
        invite_auth_attempt_count < MAX_INVITE_AUTH_ATTEMPTS)) {
    char state_log[128];
    snprintf(state_log, sizeof(state_log),
  ...
  if (strncmp(uri, "sip:", 4) != 0) {
    ...
    if (strchr(uri, '@') != NULL) {
      ...
      snprintf(formatted_uri, sizeof(formatted_uri), "sip:%s@%s", uri,
               sip_config.server);
    }
    uri = formatted_uri;
  }
  ...
  // CRITICAL FIX: Only generate NEW IDs if this is a FRESH call (no auth
  // challenge exists) If we have an auth challenge, we're retrying and must
  ...
  if (!has_invite_auth_challenge) {
    ...
    snprintf(init_log, sizeof(init_log),
             "INVITE fresh call: generating NEW Call-ID=%d, From-tag=%d, "
             "Branch=%d, CSeq=%d",
             initial_invite_call_id, initial_invite_from_tag,
             initial_invite_branch, initial_invite_cseq);
    ...
    snprintf(reuse_log, sizeof(reuse_log),
             "INVITE auth retry: REUSING Call-ID=%d, From-tag=%d (CSeq=%d), "
             "NEW Branch=%d",
             initial_invite_call_id, initial_invite_from_tag,
             initial_invite_cseq, auth_invite_branch);
    sip_add_log_entry("info", reuse_log);
  }

  // Store the Call-ID string for debugging
  snprintf(invite_call_id_str, sizeof(invite_call_id_str), "%d@%s",
           initial_invite_call_id, local_ip);

  int call_id = initial_invite_call_id;
  int tag = initial_invite_from_tag;
  int branch =
      has_invite_auth_challenge ? auth_invite_branch : initial_invite_branch;

  // Check if we have stored authentication challenge for INVITE
  if (has_invite_auth_challenge && invite_auth_challenge.valid &&
      invite_auth_attempt_count < MAX_INVITE_AUTH_ATTEMPTS) {
    sip_add_log_entry("info",
                      "Using stored INVITE authentication challenge for retry");
    ...
    char debug_msg[512];
    snprintf(debug_msg, sizeof(debug_msg),
             "INVITE Auth retry %d: nonce='%.20s...', nc='%s', cnonce='%s', "
             "Call-ID=%d, From-tag=%d",
             invite_auth_attempt_count + 1, invite_auth_challenge.nonce, nc_str,
             cnonce, call_id, tag);
    sip_add_log_entry("info", debug_msg);

    // Calculate digest response for INVITE
    char response[33];
    // CRITICAL FIX: The digest URI MUST match the Request-URI (the target being
    // called) Use the actual target URI, not our own username Limit to 64 bytes
    // to satisfy format-truncation checks
    ...
    snprintf(
        digest_inputs, sizeof(digest_inputs),
        "INVITE digest inputs: username=%s, realm=%s, method=INVITE, uri=%s, "
        "qop=%s, nc=%s",
        sip_config.username, invite_auth_challenge.realm, invite_uri_for_digest,
        invite_auth_challenge.qop[0] ? invite_auth_challenge.qop : "(empty)",
        nc_str);
    sip_add_log_entry("info", digest_inputs);

    calculate_digest_response(
        sip_config.username, sip_config.password, invite_auth_challenge.realm,
        invite_auth_challenge.nonce, "INVITE",
        invite_uri_for_digest, // Use target URI for digest calculation
        invite_auth_challenge.qop, nc_str, cnonce, response);

    // Log the calculated response for debugging
    char response_debug[128];
    snprintf(response_debug, sizeof(response_debug),
             "INVITE digest response: %s", response);
    sip_add_log_entry("info", response_debug);
    ...
    snprintf(cseq_log, sizeof(cseq_log),
             "INVITE CSeq for auth retry: %d (should be SAME as initial, not "
             "incremented)",
             initial_invite_cseq);
    ...
    len = snprintf(
        ...
        safe_uri, local_ip, branch, sip_config.username, sip_config.server, tag,
        safe_uri, call_id, local_ip,
        initial_invite_cseq, // CRITICAL FIX: Use SAME CSeq as initial INVITE,
                             // not incremented
        sip_config.username, contact_ip, sip_config.username,
        invite_auth_challenge.realm, invite_auth_challenge.nonce,
        invite_uri_for_digest, response);

    // Add qop parameters if present (NO spaces after commas)
    if (strlen(invite_auth_challenge.qop) > 0) {
      len += snprintf(invite_msg + len, sizeof(invite_msg) - len,
                      ",qop=%s,nc=%s,cnonce=\"%s\"", invite_auth_challenge.qop,
                      nc_str, cnonce);
    }
    ...
    // Add algorithm if not MD5 (NO space after comma)
    if (strlen(invite_auth_challenge.algorithm) > 0 &&
        strcmp(invite_auth_challenge.algorithm, "MD5") != 0) {
      len += snprintf(invite_msg + len, sizeof(invite_msg) - len,
...

    {
      ...
      snprintf(invite_log_msg, sizeof(invite_log_msg),
               "INVITE len=%d, strlen=%zu", len, strlen(invite_msg));
    ...
    len = snprintf(invite_msg, sizeof(invite_msg),
                   ...
                   "Content-Type: application/sdp\r\n"
                   "Content-Length: %d\r\n\r\n%s",
                   uri, local_ip, branch, sip_config.username,
                   sip_config.server, tag, uri, call_id, local_ip,
                   initial_invite_cseq, // Use stored CSeq value
                   sip_config.username, contact_ip, strlen(sdp), sdp);
  }
  ...
  if (current_state == SIP_STATE_CONNECTED ||
    ...
    if (current_state == SIP_STATE_CONNECTED && sip_socket >= 0) {
      ...
      snprintf(bye_msg, sizeof(bye_msg),
               ...
               "User-Agent: ESP32-Doorbell/1.0\r\n"
               "Content-Length: 0\r\n\r\n",
               sip_config.username, sip_config.server, contact_ip, branch,
               sip_config.username, sip_config.server, tag, sip_config.username,
               sip_config.server, call_id, local_ip, sip_config.username,
               contact_ip);
...
void sip_client_answer_call(void) {
  // This function is now a placeholder - incoming calls are answered
  // automatically in the SIP task when INVITE is received
...
void sip_client_send_dtmf(char dtmf_digit) {
  if (current_state == SIP_STATE_CONNECTED) {
    ...
    ESP_LOGW(TAG, "Cannot send DTMF - Not in connected state (%d)",
             current_state);
...
bool sip_client_test_connection(void) {
  ...
  if (!resolve_hostname(sip_config.server, &test_addr,
                        (uint16_t)sip_config.port)) {
  ...
  snprintf(reachable_msg, sizeof(reachable_msg), "SIP server %s is reachable",
           sip_config.server);
...
void sip_get_status(char *buffer, size_t buffer_size) {
  ...
  const char *state_name =
      (state < sizeof(state_names) / sizeof(state_names[0]))
          ? state_names[state]
          : "UNKNOWN";
  ...
  snprintf(buffer, buffer_size,
           ...
           "\"port\": %d"
           "}",
           state_name, user_status, state,
           sip_config.configured ? "true" : "false", server, username, apt1,
           apt2, sip_config.port);
}

sip_state_t sip_client_get_state(void) { return current_state; }

void sip_save_config(const char *server, const char *username,
                     const char *password, const char *apt1, const char *apt2,
                     int port) {
  char save_msg[128];
  snprintf(save_msg, sizeof(save_msg), "Saving SIP configuration: %s@%s",
           username, server);
...
int sip_get_log_entries(sip_log_entry_t *entries, int max_entries,
  ...
  if (xSemaphoreTake(sip_log_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    int start_index =
        (sip_log_write_index - sip_log_count + SIP_LOG_MAX_ENTRIES) %
        SIP_LOG_MAX_ENTRIES;
    ...
    while (i < sip_log_count && count < max_entries) {
      ...
      if (sip_log_buffer[index].timestamp > since_timestamp) {
        memcpy(&entries[count], &sip_log_buffer[index],
               sizeof(sip_log_entry_t));
        count++;
      }
      i++;
    }
...
const char *sip_get_server(void) { return sip_config.server; }

const char *sip_get_username(void) { return sip_config.username; }

const char *sip_get_password(void) { return sip_config.password; }

const char *sip_get_target1(void) { return sip_config.apartment1_uri; }

const char *sip_get_target2(void) { return sip_config.apartment2_uri; }
...
void sip_set_target1(const char *target) {
  if (target) {
    strncpy(sip_config.apartment1_uri, target,
            sizeof(sip_config.apartment1_uri) - 1);
...
void sip_set_target2(const char *target) {
  if (target) {
    strncpy(sip_config.apartment2_uri, target,
            sizeof(sip_config.apartment2_uri) - 1);
...
bool sip_test_configuration(void) {
  ...
  ESP_LOGI(TAG, "Testing SIP server: %s:%d", sip_config.server,
           sip_config.port);
...
bool sip_connect(void) {
  ...
  if (current_state == SIP_STATE_DISCONNECTED) {
    current_state = SIP_STATE_IDLE;
    sip_add_log_entry("info",
                      "State changed from DISCONNECTED to IDLE - Reconnecting");
  }
